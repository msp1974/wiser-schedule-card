...
import type { WiserScheduleCardConfig, ScheduleListItem, WiserEventData } from '../types';
...
export class ScheduleListCard extends SubscribeMixin(LitElement) {
    ...
    private async loadData() {
        this.supported_schedule_types = await fetchScheduleTypes(this.hass!, this.config!.hub)
        this.schedule_list = await fetchSchedules(this.hass!, this.config!.hub);
    }


    protected shouldUpdate(changedProps: PropertyValues): boolean {
        if (changedProps.has("schedule_list")) { return true }
    ...
    _scheduleClick(schedule_type: string, schedule_id: number): void {
        const myEvent = new CustomEvent('scheduleClick', { detail: { "schedule_type": schedule_type, "schedule_id": schedule_id }});
        this.dispatchEvent(myEvent);
    }
